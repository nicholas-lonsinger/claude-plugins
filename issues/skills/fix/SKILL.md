...
2. If everything left is mechanical — waiting for CI, merging, cleanup — finish it yourself with **blocking foreground** Bash calls. The CI wait has to end in a verified green on the PR's current head SHA: a bare `gh pr checks --watch` exits 0 when no checks have registered yet and when a run was cancelled, so confirm the PR's check rollup itself rather than trusting a watch's exit code. Merge per project conventions only on a verified green, and only in `--merge` mode.
