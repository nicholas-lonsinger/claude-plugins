...
2. If everything left is mechanical — waiting for CI, merging, cleanup — finish it yourself. For the CI wait, invoke the `gittools:wait-ci` skill and follow its exit-code table; it closes the races a bare watch leaves open. If gittools is not installed, do it by hand with **blocking foreground** Bash calls: re-issue `gh pr view <PR> --json statusCheckRollup -q '.statusCheckRollup | length'` until it is non-zero (a watch started before the checks register exits 0 on nothing), then run `gh pr checks <PR> --watch --fail-fast` unpiped (re-issue it on exit 8 or a Bash timeout) and confirm every rollup entry is `SUCCESS` before trusting its exit 0. Merge per project conventions only on a verified green, and only in `--merge` mode. After a merge lands, fast-forward the local default branch with the `gittools:freshen-main` skill.
